<div class="afbf_feedback_poles">
	...
	<?endif;?>
	<form id="form1" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
...
?>
				<div id="afbf_<?=mb_strtolower($arField["CODE"])?>" class="afbf_item_pole<?=$fieldClass?>">
					<label class="afbf_name<?=$nameClass?>">
                        <?if($arField["NAME"] !== "Загрузить файл"):?><?= $arField["NAME"];?><?endif?>
...
<?					if($arField["MULTIPLE"] == "Y"):?>
    <label class="upload-label">
        ...
        <span class="upload-icon"> <img src="/local/templates/dell/img/upload-icon.svg" alt=""> </span>
...
<?					endif;?>
						<div class="afbf_input_group">
							<input hidden type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[myFile][<?=$arField["CODE"]?>]" value="" class="afbf_inputtext" />
...
<?		echo bitrix_sessid_post()?>
		...
		<div style="" class="afbf_submit_block">
            <button type="submit" >
                <a> <span class="iconBlueButton">→</span>
                    Заказать
                </a>
            </button>
		</div>
